  ...
  factory: function () {
    ...
    function apply(ctx) {
      ...
      var CSS = [
        ...
        ROWS + '.dsh-pinned{',
        '  background-color: var(--dsw-alias-bg-base);',
        '  /* official fade mask is invisible on dsh flat background (mask color == bg),',
        '     so add a real drop shadow below the row for the same visible shadow look. */',
        '  /* two-layer shadow: tight near shadow + wider depth shadow; pure black so it',
        '     reads as a projection on the dark theme (never a glow) */',
