  ...
  factory: function () {
    ...
    function apply(ctx) {
      ...
      var CSS = [
        ...
        '  opacity: 0;',
        '  position: absolute;',
        '  /* cover the row box + 24px below: the row part is fully opaque (no content',
        '     shows through, no hard edge), the part below fades out so scrolled',
        '     content dissolves gradually instead of being cut by an opaque block */',
        '  inset: 0 0 -24px 0;',
        '  right: 0;',
        '  transform: translateZ(0);',
        '  /* row height (34px) is ~58.6% of the 58px mask height: solid until the',
        '     row bottom, then fade to transparent */',
        ...
        ROWS + '.dsh-pinned{',
        '  /* no flat background: the mask covers the row box (same color as the page',
        '     bg, so no visible patch) and fades out below it. A real drop shadow',
        '     gives the visible shadow the official fade mask cannot on dsh flat bg. */',
        '  box-shadow: 0 1px 2px rgba(0, 0, 0, 0.4), 0 4px 10px rgba(0, 0, 0, 0.3);',
        '  /* box-shadow with 0 x-offset bleeds to the sides too, which looks like a',
        '     raised patch; clip the sides/top so the shadow only shows below */',
