/**
 * @kongjianguan/dsh-think-sticky — browser bundle (module-factory form, mirrors
 * the dsh client loader contract used by @local/dsh-opencode-usage).
 *
 * Problem: block header rows (Think reasoning, command cards, context
 * injection, tool rows, bash rows) live at the top of their block. When an
 * expanded block is taller than the viewport, scrolling pushes the row —
 * and its expand/collapse button — out of view.
 *
 * Approach (mirrors the official DeepSeek chat UI): the row is sticky at
 * the top of the conversation scroller, but stays visually transparent.
 * While the row is actually pinned (and its block is expanded), a mask
 * layer (::after, page background color) fades in underneath the text so
 * scrolled content never shows through — no persistent background blocks,
 * no borders, no shadows.
 *
 * The pin state is tracked with a scroll listener (capture phase, rAF
 * throttled): a row is "pinned" when its box straddles the top edge of its
 * nearest scrolling container. Only expanded rows (aria-expanded="true")
 * get the mask, so collapsed rows scroll past cleanly.
 *
 * Degradation is graceful: if upstream ever wraps the row in an
 * overflow-hidden ancestor, sticky silently stops working and the row
 * scrolls away exactly as before — the mask simply never appears.
 *
 * `module.exports.__internals` is a test-only seam (see test/unit.test.mjs);
 * it carries the pure logic (CSS builder, pin predicate, scroller lookup)
 * without changing runtime behavior.
 */
window.__ModuleLoader__.load({
  id: '@kongjianguan/dsh-think-sticky',
  factory: function () {
    var module = { exports: {} }

    var ROWS_A = '[data-slot="conversation.session"] [data-disclosure-row]'
    var ROWS_B = '[data-slot="conversation.session"] [data-variant="bash"][role="button"]'
    var ROWS = ROWS_A + ',' + ROWS_B
    // Selector lists cannot share a pseudo-element suffix — each selector must
    // carry its own, otherwise the bare element also receives the pseudo styles
    // (position:absolute would clobber sticky). `suffixed` makes that structural.
    function suffixed(suffix) {
      return ROWS_A + suffix + ',' + ROWS_B + suffix
    }
    var BG = suffixed('::after')
    var FADE = suffixed('::before')
    var FADE_PINNED = suffixed('.dsh-pinned::before')

    function buildCss() {
      return [
        '/* dsh-think-sticky — sticky disclosure rows:',
        '   - row: sticky, z-index 7, opaque via ::after bg layer',
        '   - ::before: 24px fade mask below the row, opacity driven by JS',
        '     while pinned */',
        ROWS + '{',
        '  position: sticky;',
        '  top: 0;',
        '  z-index: 7;',
        '  /* taller pinned header: 24px content + 5px*2 padding = 34px, matching official --collapsible-area-title-height */',
        '  padding-block: 5px;',
        '}',
        /* persistent opaque background layer — behind the row content.
           The official UI paints a 300px sweep-glow on the same ::after while
           the block is streaming (`.QWLzlG_root[data-state=running]
           .QWLzlG_row:after`, specificity (0,3,1) — higher than ours), whose
           `background` shorthand resets background-color to transparent and
           clamps the layer to 300px. !important keeps the mask opaque and
           full-width under that rule so pinned text never shows scrolled
           content through. */
        BG + '{',
        '  content: "";',
        '  pointer-events: none;',
        '  width: calc(100% + 10px) !important;',
        '  background-color: var(--dsw-alias-bg-base) !important;',
        '  z-index: -1 !important;',
        '  height: calc(100% + 1px) !important;',
        '  position: absolute !important;',
        '  top: -1px !important;',
        '  right: 0 !important;',
        '  /* official running-sweep rule also pins left:0 — undo it so the',
        '     full-width mask stays anchored to the right edge as designed */',
        '  left: auto !important;',
        '}',
        /* fade mask below the row — official .c99b79f8 */
        FADE + '{',
        '  content: "";',
        '  pointer-events: none;',
        '  width: calc(100% + 10px);',
        '  /* gradient fade: solid bg at the row bottom dissolving to transparent —',
        '     scrolled text melts instead of hitting a hard edge */',
        '  background: linear-gradient(var(--dsw-alias-bg-base), transparent);',
        '  opacity: 0;',
        '  height: 24px;',
        '  position: absolute;',
        '  top: calc(100% - 1px);',
        '  right: 0;',
        '  transition: opacity 120ms ease-out;',
        '}',
        FADE_PINNED + '{',
        '  opacity: 1;',
        '}',
        ROWS + '.dsh-pinned{',
        '  /* the row classes are overflow:hidden, which would clip the',
        '     absolutely-positioned ::before fade band to the row box —',
        '     let it extend below while pinned */',
        '  overflow: visible !important;',
        '}'
      ].join('\n')
    }

    /** True when the row box straddles the top edge of its scroller. */
    function isPinned(rowRect, scRect) {
      return rowRect.top <= scRect.top + 2 && rowRect.bottom > scRect.top + 2
    }

    /** Nearest scrollable ancestor (or the document root fallback). */
    function scrollerOf(el) {
      var p = el.parentElement
      while (p) {
        var cs = getComputedStyle(p)
        var ov = cs.overflowY
        if ((ov === 'auto' || ov === 'scroll') && p.scrollHeight > p.clientHeight + 1) {
          return p
        }
        p = p.parentElement
      }
      return document.scrollingElement || document.documentElement
    }

    function apply(ctx) {
      // Idempotent: skip if the same style tag is already present (HMR reloads).
      if (document.querySelector('style[data-plugin="dsh-think-sticky"]') === null) {
        var styleTag = document.createElement('style')
        styleTag.setAttribute('data-plugin', 'dsh-think-sticky')
        styleTag.textContent = buildCss()
        document.head.appendChild(styleTag)
      }

      var ticking = false

      function update() {
        ticking = false
        var rows = document.querySelectorAll(ROWS)
        for (var i = 0; i < rows.length; i++) {
          var row = rows[i]
          var sc = scrollerOf(row)
          var scRect = sc.getBoundingClientRect()
          var rect = row.getBoundingClientRect()
          var expanded = row.getAttribute('aria-expanded') === 'true'
          row.classList.toggle('dsh-pinned', isPinned(rect, scRect) && expanded)
        }
      }

      function schedule() {
        if (ticking) return
        ticking = true
        requestAnimationFrame(update)
      }

      document.addEventListener('scroll', schedule, true)
      window.addEventListener('resize', schedule)
      window.addEventListener('load', schedule)
      update()

      ctx.effect(function () {
        return function () {
          document.removeEventListener('scroll', schedule, true)
          window.removeEventListener('resize', schedule)
          window.removeEventListener('load', schedule)
          var rows = document.querySelectorAll(ROWS)
          for (var i = 0; i < rows.length; i++) rows[i].classList.remove('dsh-pinned')
          if (styleTag && styleTag.parentNode) styleTag.parentNode.removeChild(styleTag)
        }
      })
    }

    module.exports = { name: 'dsh-think-sticky', apply: apply }
    // Test-only seam — pure logic, no DOM access at definition time.
    module.exports.__internals = {
      ROWS: ROWS,
      BG: BG,
      FADE: FADE,
      FADE_PINNED: FADE_PINNED,
      buildCss: buildCss,
      isPinned: isPinned,
      scrollerOf: scrollerOf
    }
    return module.exports
  }
})
